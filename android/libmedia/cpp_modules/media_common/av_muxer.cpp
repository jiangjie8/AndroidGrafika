...
namespace av {
    ...
    int AVMuxer::addStream(AVCodecParameters *parm){
        ...
        if(stream == nullptr){
            ALOGE("avformat_new_stream error");
            return -1;
        }
    ...
    int AVMuxer::writeHeader(){
        ...
        if (!(ofmt_ctx->flags & AVFMT_NOFILE)) {
            ...
            if (ret < 0) {
                ALOGE("Could not open output file '%s'", m_output.c_str());
                return -1;
            }
        }
...
//        ALOGE("index %d pst   %lld  size %d  flags %d ",packet->stream_index, packet->pts, packet->size, packet->flags);
        av_packet_rescale_ts(packet, AV_TIME_BASE_Q, stream_timebae);
        ret = av_interleaved_write_frame(m_outputFormat.get(), packet);
        if (ret < 0) {
            ALOGE("write packet error");
            return ret;
        }
        return ret;
    }
